...
if [[ "$MODE" == "external-pulsar" ]];then
  sed -i s/"clusterName=.*"/"clusterName=xavki-geo"/g /opt/pulsar/conf/broker.conf
else
...
if [[ $PULSAR_ID == "3" ]] || [[ "${MODE}" == "external-pulsar" ]];then
  ...
  if [[ ${MODE} == "external-pulsar" ]];then
    ...
    /opt/pulsar/bin/pulsar-admin --admin-url http://127.0.0.1:8082 topics create-partitioned-topic geotenant/geons/xavki-topic-geo -p 6
  else
    ...
    /opt/pulsar/bin/pulsar-admin --admin-url http://127.0.0.1:8082 topics create-partitioned-topic xtenant/xns/xavki-topic -p 6
  fi
  sleep 15s
fi
