...
sed -i s/spring.datasource.username=.*/spring.datasource.username=xavki/g application.properties
...
curl    -H 'X-XSRF-TOKEN: $CSRF_TOKEN'    -H 'Cookie: XSRF-TOKEN=$CSRF_TOKEN;'    -H "Content-Type: application/json"    -X PUT http://127.0.0.1:7750/pulsar-manager/users/superuser    -d '{"name": "xavki", "password": "password", "description": "test", "email": "username@test.org"}'
}
